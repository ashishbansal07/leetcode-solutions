...
const deleteAndEarn = function(nums) {
    ...
    let occurence = new Array(max+1);
    occurence.fill(0);
    
    for(let i=0;i<nums.length;i++) {
        let current = nums[i];
        occurence[current] += current;
    }
    
    for(let i=2; i<occurence.length;i++) {
        occurence[i] = Math.max(occurence[i-1], occurence[i]+occurence[i-2]);
    }
    
    return occurence[max];
};